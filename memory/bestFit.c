    ...
    for (int i = 0; i < num_processes; i++) {
        ...
        for (int j = 0; j < num_blocks; j++) {
            if (block_sizes[j] >= process_sizes[i]) {
                if (best_index == -1 || block_sizes[j] < block_sizes[best_index]) {
                    best_index = j;
                }
            }
        }
        ...
        if (best_index != -1) {
            allocation[i] = best_index + 1; // Allocate block (index + 1 for 1-based index)
            block_sizes[best_index] -= process_sizes[i]; // Reduce available block size
        }
    }
